...
setwd('c:/Users/karsten/Dropbox/Devel/CPTAC-LUSC2020/')

## import gct files
data.dir <- 'g:/Shared drives/CPTAC3.0_LSCC_Data/lscc-v2.0-data-freeze/'
label <- 'v2.0-tumor-over-nat'

ord.column <- 'NMF.cluster'


gct.str <- c(glue("{data.dir}lscc-v2.0-acetylome-ratio-norm-NArm.gct"),
             glue("{data.dir}lscc-v2.0-gene-level-cnv-gistic2-all_data_by_genes.gct"),
             glue("{data.dir}lscc-v2.0-phosphoproteome-ratio-norm-NArm.gct"),
             glue("{data.dir}lscc-v2.0-proteome-ratio-norm-NArm.gct"),
             glue("{data.dir}lscc-v2.0-rnaseq-uq-fpkm-log2-NArm-row-norm.gct")
             )
...
for(i in names(gct)){
  ...
  if('NAT' %in% cdesc$Type){
    ...
    #t.cid <- t.cid[ match( sub('\\.N$','', n.cid), t.cid ) ] %>% na.omit
    mat.t <- mat.t[, t.cid]
    mat.n <- mat.n[, n.cid]
...
# #################
# ## harmonize missing CIMP status
# idx <- which(cdesc$CIMP.status == '')
# if(length(idx) > 0){
#   cdesc$CIMP.status[idx] <- NA
# }
# cdesc$CIMP.status[ is.na( cdesc$CIMP.status ) ] <- 'NA' 

## fix smoking status
cdesc$Smoking.score <- as.numeric(cdesc$Smoking.score)
cdesc$Estimate_ImmuneScore.rna <- as.numeric(cdesc$Estimate_ImmuneScore.rna)
